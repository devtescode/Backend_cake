...
router.post("/webhook", express.raw({ type: "application/json" }), async (req, res) => {
  try {
    ...
    if (event.event === "charge.success") {
      ...
      await payment.save();
      console.log(`💰 Payment saved: ${reference} (${currencyCode} ${amountInFullCurrency})`);
      ...
      if (user) {
        ...
        user.Balance[currencyCode] += amountInFullCurrency;
        await user.save();

        console.log(`✅ ${currencyCode} balance updated for ${user.fullname}: +${amountInFullCurrency}`);
      } else {
